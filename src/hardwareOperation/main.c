...
// biquad_state:     12ch × 3 filters × 4 coeffs × 8 bytes = 1,152
// rms_seq:          8 × N_FEATURES × 4 = 384
// cfc_scratch:      ~4,096 (hidden states + intermediate)
// TOTAL:            ~51.2 KB of 400 KB SRAM budget
...
static void init_dsp_filters(void) {
    for (int ch = 0; ch < N_CHANNELS; ch++) {
        ...
        dsps_biquad_gen_notch_f32(notch_coeffs[ch], 50.0f / (float)ADC_FS, 0.707f);
...
static void adc_isr_task(void *arg) {
    ...
    for (int ch = 0; ch < N_CHANNELS; ch++) {
        ...
        for (int i = 0; i < (int)(out_len / SOC_ADC_DIGI_RESULT_BYTES); i++) {
            ...
            int16_t val = (int16_t)(p->type1.data & 0xFFF);  // 12-bit
...
void app_main(void) {
    ...
    printf("SRAM budget: ~52 KB / 400 KB\n");
    ...
    //    CfC task      |  0   |  low     | 8192   | cfc_task_handle
    //    Output task   |  0   |  lowest  | 2048   | (none)

    xTaskCreatePinnedToCore(dsp_task,    "dsp",    8192, NULL, 5,
                            &dsp_task_handle, 0);
    xTaskCreatePinnedToCore(cfc_task,    "cfc",    8192, NULL, 3,
                            &cfc_task_handle, 0);
